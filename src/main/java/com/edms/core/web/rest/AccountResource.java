...
import com.edms.core.service.dto.PasswordChangeDTO;
import com.edms.core.service.dto.UserDTO;
import com.edms.core.web.rest.errors.*;
import com.edms.core.web.rest.vm.KeyAndPasswordVM;
import com.edms.core.web.rest.vm.ManagedUserVM;
...
public class AccountResource {
	...
	public void activateAccount(@RequestParam(value = "key") String key) {
		...
		if (!user.isPresent()) {
			throw new AccountResourceException(ConstantUtils.NO_USER_WAS_FOUND_ACTIVATIONKEY);
		}
	}
	...
	public UserDTO getAccount() {
		return userService.getUserWithAuthorities().map(UserDTO::new)
				.orElseThrow(() -> new AccountResourceException(ConstantUtils.USER_COULD_NOT_BE_FOUND));
	}
	...
	public void logOut() {
		UserDTO userDTO = userService.getUserWithAuthorities().map(UserDTO::new)
				.orElseThrow(() -> new AccountResourceException(ConstantUtils.USER_COULD_NOT_BE_FOUND));
		userService.updateLogoutDate(userDTO.getLogin());
	...
	public void saveAccount(@Valid @RequestBody UserDTO userDTO) {
		String userLogin = SecurityUtils.getCurrentUserLogin()
				.orElseThrow(() -> new AccountResourceException(ConstantUtils.CURRENT_USER_LOGIN_NOT_FOUND));
		...
		if (!user.isPresent()) {
			throw new AccountResourceException(ConstantUtils.USER_COULD_NOT_BE_FOUND);
		}
	...
	public void finishPasswordReset(@RequestBody KeyAndPasswordVM keyAndPassword) {
		...
		if (!user.isPresent()) {
			throw new AccountResourceException(ConstantUtils.USER_NOT_FOUND_RESETKEY);
		}
	}
